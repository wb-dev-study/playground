...
public class GameRulesUtils {
    ...
    private static final int CAR_NAME_LENGTH_MIN_LIMIT = 1;
    ...
    public static boolean validateCarNameLength(String carName) {
        if (carName.length() <= CAR_NAME_LENGTH_MAX_LIMIT && carName.length() >= CAR_NAME_LENGTH_MIN_LIMIT) return true;

        throw new IllegalArgumentException("자동차 이름은 1 ~ 5글자까지 가능합니다. ex: 가나다라마");
    }

    public static boolean checkDuplicateName(int order, String name, String[] carNames) {
